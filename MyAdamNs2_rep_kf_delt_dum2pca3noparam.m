function [theta errors  gradients LL non_scaled hh2 u alf smgd gain tm1 tm2] = MyAdamNs2_rep_kf_delt_dum2pca3noparam(sigma02,a,glmt,sigma_beta2,delt,pi1,M,kf)
...
%%
  ...
  % q=rand(1,M);
   ...
   mnx=mean(A3);
   %data=A3-mnx;
  [data,mu,sigma]=zscore(A3);
   
   
  
   %[coeff,score,latent,tsquared,explained] = pca(data);
   [coeff,latent,explained]  = mypcacov(cov(data));

   [sh,ind] = sort(explained,'descend');  
   K1=cumsum(sh);
   
   %K1=cumsum(explained);
   %K1=cumsum(eigenvalue);
   K11=find((K1/100)>pca_thr);
   %K11=find(K1>pca_thr);
   K=3*K11(1)
   gain=K/M;
...
%    %[eigvec1,eigval1] = eig(A_term2);
%    [sh,ind] = sort((diag((eigval))),'descend');
%    K1=cumsum(abs(sh))/sum(abs(sh));
%    K11=find(K1>pca_thr);
%    K=K11(1);
%    mnx=mu;
%    
...
%    
   ...
   tm1=0;
   tm2=0;
   while ( iteration<10*10^3 &&   sumgrad>.25*10^-3 )    
 ...
 %   alfa=2*alphaB/sqrt(count/4);
    ...
    g1x=-1*g111_new_delt_pca2(sigma02,a,glmt,m,q,s,sigma_beta2,deltOpt,pi1,u,kf,A_term1,A_term2,C_term,eigs,mnx,D_term,mu);
...
if count>10
sumgrad=sum(abs(thtnew-thtold))+sum(abs(btanew-btaold));
end
% if count>2
...
%LL=Lfunc(sigma02,a,glmt,m,1./(1+exp(-u)),s,sigma_beta2,delt,pi1);
...
if iteration>6000 && mod(iteration,100)==0

pi1=(1/M)*sum(1./(1+exp(-kf*u))); % optimized by derivative

sigma_beta2=2*sum((m.^2 + s.^2).*(1./(1+exp(-kf*u))))/(M*pi1);

TA=0*sum(a.^2*(s.^2)')+sum((glmt-(a*m')).*(glmt-(a*m')));
