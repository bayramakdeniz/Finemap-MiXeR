function FinemapMiXeRv0(a,glmt)
...
%hyperparameters (will be optimized during the algorithm)
pi1=0.001;    
sigma02=1;
sigma_beta2=0.005;
adj=0.15;
...
%    eigs=eigvec(:,ind(1:K))';
   ...
   while ( iteration<30*10^3 &&   sumgrad>2.5*10^-3 )    
...
mm=real(max(10^-27,min(theta(M+1:2*M))));



theta(M+1:2*M)=rescale(real(theta(M+1:2*M)),mm,max(real(theta(M+1:2*M))));
...
if iteration>4000 && mod(iteration,100)==0
...
variance_effect=theta(M+1:2*M);

save("resultsFinemapMixer.mat","probs","effect_size","variance_effect");
